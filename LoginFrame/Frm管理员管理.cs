...
namespace LoginFrame
{
    public partial class Frm管理员管理 : Form
    {
        ...
        void dataBind()
        {
            DataSet ds = new BU_UserInfo().SelectAdmin("U_Id<>1");//查询管理员
        ...
        private void treeView1_NodeMouseClick_1(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            if (data.Length == 2)
            {
                ...
                if (data[1] == "男")
                {
                    ds = new BU_UserInfo().SelectAdmin("U_Sex=0 and U_Id<>1");//查询普通用户
                }
                else
                {
                    ds = new BU_UserInfo().SelectAdmin("U_Sex=1 and U_Id<>1");//查询普通用户
                }
                dataGridView1.DataSource = ds.Tables[0].DefaultView;
            }
        }
