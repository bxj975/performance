...
namespace DAL
{
    public class DU_UserInfo
    {
        ...
        public DataSet ExistsPwd(string U_Name, string pwd)
        {
            ...
            strSql.Append(" where ");
            strSql.Append(" U_Name2 = @U_Name and U_PassWord=@U_PassWord ");
            SqlParameter[] parameters = {
					new SqlParameter("@U_Name", SqlDbType.VarChar,50),
        ...
        public DataSet ShowPwd(string U_Name, string pwd)
        {
            ...
            strSql.Append(" where ");
            strSql.Append(" U_Name2 = @U_Name and U_PsdAnswer=@U_PsdAnswer ");
            SqlParameter[] parameters = {
					new SqlParameter("@U_Name", SqlDbType.VarChar,50),
        ...
        /// <returns></returns>
        public DataSet SelectAdminAAA(string where)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select U_Id as 用户编号,");
            strSql.Append(" U_Name2 as 用户名,U_RelName as 真实姓名,(case  U_Sex  when '0' then '男' when '1' then '女'  else ''  end  ) as 性别,");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select U_Id as ID,");
            strSql.Append(" U_Name2 as 用户名,U_RelName as 姓名,(case  U_Sex  when '0' then '男' when '1' then '女'  else ''  end  ) as 性别,");
            strSql.Append(" U_Telephone as 联系电话");
            strSql.Append(" from U_UserInfo");
            if (where != "")
            {
                strSql.Append(" where " + where);
